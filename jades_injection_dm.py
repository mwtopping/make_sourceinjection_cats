...
from generate_SED import normalize_flux_in_filter
...
def create_parser():
    ...
    parser.add_argument('--powerlaw', type=int, default=0, help='ayudame')
...
def populate_from_distributions(icats):
    ...
    filts = icats['FLUX'].header["FILTERS"].split(",")
    ...
    print("Populating source properties")
    icat = icats["SHAPE"]
    print("...Sersic Index")
    icat.data["sersic"] = truncnorm((ns_lower - ns_mu) / ns_sigma,
                               (ns_upper - ns_mu) / ns_sigma, loc=ns_mu,
                               scale=ns_sigma).rvs(size=ngals, random_state=seed)
    print("...Q")
    #icat.data['q']  = np.random.uniform(0.2, 1.0, size=ngals)
    icat.data['q']  = np.ones(shape=ngals)


    print("...PA")
   # icat.data['pa']  = np.random.uniform(-90, 90, size=ngals)

    print("...Muv") 
    muv = sample_muv(-25, -15, -20, 2, ngals)
    print("...Z")
    redshifts = np.random.uniform(7, 12, ngals)

    icats['PHYSICAL'].data['redshift'] = redshifts

    print("...Rhalf")
    rhalfs = (shibuya_rcirc(muv, redshifts)*cosmo.arcsec_per_kpc_proper(redshifts)).value
    icat.data["rhalf"] = rhalfs

    icat = icats["FLUX"]
    print("...UV slopes")
    betas = uv_slope(redshifts, muv, 'F115W')

    print("...Fluxes")
    for ii in tqdm(range(ngals)):
        model_fluxes = normalize_flux_in_filter(redshifts[ii], muv[ii], betas[ii], filts)
        for f in filts:
            icat.data[f][ii] = model_fluxes[f]
...
def make_and_populate(args):
    ...
    mosaic_path = f"/media/michael/Data/JWST/NIRCam/JADES/GOODSS/mosaic_F444W.fits"
